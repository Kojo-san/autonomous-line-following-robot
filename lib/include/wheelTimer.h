 ...
 * Section 3
 * 
 * Description: Cette classe permet de configurer et de contrôler un Timer pour des roues ou moteurs, 
 *              avec des modes tels que CTC, PWM rapide et phase correcte. Elle offre des méthodes pour 
 *              régler les délais, les interruptions, le prescaler, la valeur TOP et les registres OCR2A et OCR2B.
 * 
 */
 ...
 public:
     ...
     static constexpr uint8_t DEFAULT_TOP_VALUE_TICKS = 255;
     ...
     static void initFastPWM(uint16_t prescaler, bool inverting);
     ...
     static void setOCR2A(uint8_t value_ticks);
     static uint8_t getOCR2A();
     static void setOCR2B(uint8_t value_ticks);
     static uint8_t getOCR2B();
