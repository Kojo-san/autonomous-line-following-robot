...
# warning "F_CPU pas defini pour 'memory_24.cpp'"
# define F_CPU 8000000UL
#endif


uint8_t Memory24CXXX::m_device_address = 0xA0;
...
/******************************************************************************/
uint8_t Memory24CXXX::select_bank(const uint8_t bank)
{
   uint8_t temp = bank & 0x03;
   uint8_t rv = 255;
   if(bank == temp)
   {
      Memory24CXXX::m_device_address = (0xA0 | ( bank << 1 ));
      rv = Memory24CXXX::m_device_address;
   }
   return rv;
}
...
uint8_t Memory24CXXX::read(const uint16_t address, uint8_t *data)
{
  ...
  for (;;)
  {
    ...
    TWDR = m_device_address;    //controle - bit 0 a 0, ecriture
  ...
  //__________________ Transmission du code de controle ___________________
  TWDR =  m_device_address + 1;   // Controle - bit 0 a 1 lecture 
  TWCR = _BV(TWINT) | _BV(TWEN);        // R. a Z., interrupt. - Depart de transm.
  while ((TWCR & _BV(TWINT)) == 0)      // Attente de fin de transmission
    ;
...
uint8_t Memory24CXXX::write(const uint16_t address, uint8_t *data,
                             const uint8_t length)
{
  uint8_t rv;
  uint16_t copyAddress = address;
  uint8_t copyLength = length;
  do
  {
      rv = write_page(copyAddress, data, copyLength);
      copyAddress  += rv;      // On pointe une nouvelle page
      copyLength  -= rv;     // On soustrait la partie ecrite precedemment
...
uint8_t Memory24CXXX::write_page(const uint16_t address, uint8_t *data,
                                  const uint8_t length)
{
  ...
  if (address + length < (address | (PAGE_SIZE - 1)))
    ...
    end_address = (address | (PAGE_SIZE - 1)) + 1;
  copyLength = end_address - address;
